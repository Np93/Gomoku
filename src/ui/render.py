...
def draw_forbidden_message(message):
	...
	wrapping text if needed to avoid going outside the panel.
	"""
	font = pygame.font.Font(None, 32)
	max_width = score_panel_width - 20
	words = message.split()
	lines = []
	current_line = ""

	# Split text by words to avoid exceeding max_width
	for word in words:
	...
	# Draw each line
	message_x = screen_size + 2 * border_size
	message_y = 300
	line_spacing = 35
...
def draw_button(screen, button_rect, text, font, mouse_pos, button_color, hover_color, text_color):
	"""Draw a rectangular button with hover effect and centered text."""
...
def draw_quit_button(screen, quit_center, radius, font, text_color, button_color):
	...
	screen.blit(
		quit_text,
		(
			quit_center[0] - quit_text.get_width() // 2,
...
def main_menu():
	...
	while True:
		...
		duo_button = pygame.Rect(total_screen_width // 2 - 100, 500, 200, 50)
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if normal_button.collidepoint(mouse_pos):
					...
					return "special"
				elif duo_button.collidepoint(mouse_pos):
					return "duo"
...
def draw_board(gomoku, winner=None, forbidden_message=None):
	"""
	Draw the board using the C++ Gomoku data (via getters).
	"""
	screen.fill(BORDER_COLOR)
	...
	for row in range(board_size):
		for col in range(board_size):
			...
			if value == PlayerToken.WHITE.value:  # -1
				pygame.draw.circle(
					screen,
					WHITE,
					(grid_start + col * cell_size, grid_start + row * cell_size),
					int(pion_radius),
				)
			elif value == PlayerToken.BLACK.value:  # 1
				pygame.draw.circle(
					screen,
					BLACK,
					(grid_start + col * cell_size, grid_start + row * cell_size),
	...
	black_score_text = font.render(
		f"Pions pris par Noir : {gomoku.getBlackPlayerPebblesTaken()}",
		True,
		TEXT_COLOR,
	)
	white_score_text = font.render(
		f"Pions pris par Blanc : {gomoku.getWhitePlayerPebblesTaken()}",
		True,
		TEXT_COLOR,
	)
	next_player_text = font.render(
		f"Prochain joueur : {next_player}",
		True,
		BLACK if current_player == PlayerToken.BLACK.value else WHITE,
	)
	...
	# Display forbidden move message if necessary
	if forbidden_message:
		draw_forbidden_message(forbidden_message)

	# If winner is set, display a center message
	if winner:
		winner_text = font.render(f"Le Joueur {winner} a gagné !", True, WINNER_COLOR)
...
def render_game_ui():
	...
	while not exit_game:
		...
		if game_mode in ["normal", "duo", "special"] and not exit_game:
			# Instantiate the C++ Gomoku object:
			gomoku = Gomoku()
			running = True
			game_over = False
			winner = None

			# By default, let's pick which player is the AI (for "special" mode).
			...
			# Initialize an AI object (Python side) if you still use your existing GomokuAI class

			while running and not exit_game:
				draw_board(gomoku, winner, forbidden_message)

				# Expire forbidden message after message_duration
				...
				pygame.display.flip()
				# Check if it's AI's turn in "normal" or "special" mode
				if game_mode in ["normal", "special"] and gomoku.getCurrentPlayer() == ia_player:
					ai = GomokuAI(gomoku=gomoku)
					if not turn_start_time:
						turn_start_time = time.time()

					# Example of using your Python-based AI with the C++ gomoku
					score, best_move = ai.minmax(3, True, True)
					if best_move:
						row, col = best_move

						# Update timing for the current player
						if turn_start_time:
							turn_duration = time.time() - turn_start_time
							player_times[gomoku.getCurrentPlayer()]["total_time"] += turn_duration
							player_times[gomoku.getCurrentPlayer()]["last_time"] = turn_duration
							turn_start_time = None

						# processMove returns (bool, forbidden_message)
						is_valid, message = gomoku.processMove(row, col)
						if not is_valid:
						...
						if gomoku.getGameStatus():
							game_over = True
							cur_p = gomoku.getCurrentPlayer()
							winner = "Noir" if cur_p == PlayerToken.BLACK.value else "Blanc"
				...
				# If game is over, display end menu
				if game_over and not exit_game:
					action = end_game_menu(winner)
					if action == "replay":
						# Re-create the C++ Gomoku instance
						gomoku = Gomoku()
						# Re-initialize the AI if needed
						ai = GomokuAI(gomoku=gomoku)
						...
						# If "special", randomly choose which player the AI will be again
						if game_mode == "special" and not exit_game:
							ia_player = random.choice([PlayerToken.BLACK.value, PlayerToken.WHITE.value])
					elif action == "menu":
						break
